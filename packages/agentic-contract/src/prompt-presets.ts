...
export const DETERMINISTIC_ENGINE_POLICIES: DeterministicEnginePolicy[] = [
  {
    ...
    policy: `Every result must come from a recorded computation, never prose arithmetic. Do not silently change approved assumptions or clamp an invalid value. Halt when the discount rate does not exceed terminal growth, when currencies or inflation bases conflict, or when required inputs are missing. FCFF must discount at WACC and FCFE at cost of equity; never mix them. Keep country risk either in the local beta or as a separately documented global-beta adjustment, never both. Require a stated business driver for revenue growth and margins, and a stated rationale when working capital does not move with growth. Prevent double counting across debt, leases, options, diluted shares, tax assets, and non-operating assets. Apply one explicit timing convention consistently. Report a range and sensitivity surface rather than presenting a point estimate as precise. Explain terminal-value concentration and fragility in economic language and keep the result conditional on its assumptions.`,
  },
  {
    engineKind: 'valuation_engine',
    name: 'Deterministic comparable-company engine',
    sourceFile: 'user-supplied comparable-company prompt',
    implementationNote: 'Peer selection and inputs are human-reviewed. Enterprise value, trading multiples, summary statistics, outlier flags, and implied values are calculated deterministically.',
    scope: 'Calculate a transparent comparable-company valuation after approved research, using six to ten sourced public peers and target financial data.',
    policy: `Require six to ten peers with human-reviewed business-model, scale, and growth comparability. Exclude distressed or corporate-action-affected peers through the human review gate; do not infer those judgments. Enterprise value equals market capitalization plus net debt, minority interest, and preferred stock. Calculate EV / Revenue, EV / EBITDA, and P / E only when the relevant positive denominator is supplied. Report count, mean, median, 25th percentile, and 75th percentile. Flag IQR outliers for review rather than removing them automatically. Apply mean and median multiples to only supplied target metrics, bridge EV to equity value using supplied target net debt, and never state a valuation as a price target or trade instruction.`,
  },
  {
