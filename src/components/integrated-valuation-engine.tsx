...
function sample(kind: 'tech' | 'industrials'): LabState {
  ...
  const peers = rows.map(([companyName, ticker, sharePrice, dilutedShares, totalDebt, cash, revenue, ebitda, netIncome], index) => ({
    id: `${kind}-${index}`, companyName: String(companyName), ticker: String(ticker), currency: 'USD', sharePrice: Number(sharePrice), dilutedShares: Number(dilutedShares),
...
export function IntegratedValuationEngine() {
  ...
  function addPeer() {
    setModel((current) => ({ ...current, peers: [...current.peers, { id: `peer-${Date.now()}`, companyName: 'New Peer', ticker: 'NEW', currency: current.currency, sharePrice: 0, dilutedShares: 0, totalDebt: 0, cash: 0, revenue: 0, ebitda: 0, netIncome: 0, included: true }] }));
  }
  ...
  return <section className="integrated-engine" aria-labelledby="integrated-engine-title">
    ...
    {tab === 'summary' && <div className="engine-view" id="integrated-tab-panel" role="tabpanel" aria-labelledby="integrated-tab-summary" aria-live="polite">
      ...
      <div className="engine-print-only"><h4>Comparable-company inputs and calculations</h4><div className="table-scroll"><table><thead><tr><th>Company</th><th>Ticker</th><th>Currency</th><th>Included</th><th>Equity value</th><th>EV</th><th>EV/Revenue</th><th>EV/EBITDA</th><th>P/E</th></tr></thead><tbody>{comps.rows.map((peer) => <tr key={peer.id}><td>{peer.companyName}</td><td>{peer.ticker}</td><td>{peer.currency}</td><td>{peer.included ? 'Yes' : 'No'}</td><td>{amount(peer.currency, peer.equityValue, 0)}</td><td>{amount(peer.currency, peer.enterpriseValue, 0)}</td><td>{peer.evRevenue?.toFixed(2) ?? 'N/A'}</td><td>{peer.evEbitda?.toFixed(2) ?? 'N/A'}</td><td>{peer.pe?.toFixed(2) ?? 'N/A'}</td></tr>)}</tbody></table></div>
    ...
    {tab === 'comps' && <div className="engine-view" id="integrated-tab-panel" role="tabpanel" aria-labelledby="integrated-tab-comps"><p className="note">Edit peer inputs directly; values and summary statistics recalculate immediately. Negative or zero EBITDA/net income produces N/A and is excluded from that multiple’s statistics.</p>
      <div className="table-scroll"><table className="integrated-peer-table"><thead><tr><th>Include</th><th>Company</th><th>Ticker</th><th>Currency</th><th>Price</th><th>Diluted shares</th><th>Debt</th><th>Cash</th><th>Revenue (LTM)</th><th>EBITDA (LTM)</th><th>Net income (LTM)</th><th>Equity value</th><th>EV</th><th>EV/Rev</th><th>EV/EBITDA</th><th>P/E</th><th>Remove</th></tr></thead><tbody>{comps.rows.map((peer) => <tr key={peer.id}>
        ...
        {(['companyName', 'ticker', 'currency'] as const).map((key) => <td key={key}><input aria-label={`${key} ${peer.ticker}`} value={peer[key]} maxLength={key === 'currency' ? 3 : undefined} onChange={(event) => updatePeer(peer.id, key, key === 'currency' ? event.target.value.toUpperCase() : event.target.value)} /></td>)}
        ...
        <td>{amount(peer.currency, peer.equityValue, 0)}</td><td>{amount(peer.currency, peer.enterpriseValue, 0)}</td><td>{peer.evRevenue?.toFixed(1) ?? 'N/A'}</td><td>{peer.evEbitda?.toFixed(1) ?? 'N/A'}</td><td>{peer.pe?.toFixed(1) ?? 'N/A'}</td>
